...
- Treat `scripts/scenes/s_makeSimpleScene.m` and `scripts/scenes/s_scenesRecipe.m` as likely overview scripts that may overlap with narrower scene scripts.
- Check rendering scripts as a cluster: `s_autoExample.m`, `s_batchRendering.m`, and `s_sceneFormat.m`.
- Check geometry and asset scripts as a cluster: `s_cornellBox.m`, `s_flatSurface.m`, `s_landscape.m`, and `s_makeChessSet.m`.
- Compare scene scripts against tests in adjacent `_tests_` directories.
